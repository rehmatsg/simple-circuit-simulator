...
test("golden: single resistor", async () => {
  ...
  approx(result.componentCurrents.R1 as number, 0.01);
});
...
test("golden: series resistors", async () => {
  ...
  approx(result.componentCurrents.R1 as number, 0.004);
  approx(result.componentCurrents.R2 as number, 0.004);
});
...
test("golden: parallel resistors", async () => {
  ...
  const result = solveDC(imported.circuit, { registry });
  assert.equal(result.status, "ok");
  approx(result.componentCurrents.R1 as number, 0.006);
  approx(result.componentCurrents.R2 as number, 0.003);
});
...
test("golden: switch open", async () => {
  ...
  const result = solveDC(imported.circuit, { registry });
  assert.equal(result.status, "ok");
  approx(result.componentCurrents.R1 as number, 0);
});
...
test("golden: switch closed", async () => {
  ...
  assert.equal(result.status, "ok");
  assert.ok((result.componentCurrents.R1 as number) > 0);
});
...
test("golden: current source into resistor", async () => {
  ...
  approx(result.componentCurrents.I1 as number, 0.01);
  approx(result.componentCurrents.R1 as number, 0.01);
});
...
test("golden: floating circuit without ground", async () => {
  ...
  assert.ok(
    result.errors.some(
      (err) =>
        err.code === DiagnosticCodes.floatingReference ||
