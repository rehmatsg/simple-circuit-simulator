...
const approxValue = (value: number | undefined, expected: number, epsilon = 1e-6) => {
...
test("solveDC computes voltage divider", () => {
  ...
  approx(result.componentCurrents.R1 as number, 0.005);
  approx(result.componentCurrents.R2 as number, 0.005);
});
